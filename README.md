...
- Node.js контейнер автоматически запускает Vite dev-сервер при старте
- Hot reload работает автоматически
- Ничего дополнительно делать не нужно

**Для продакшена (сборка статических файлов):**
```bash
...
**Важно:** 
- ✅ **Рекомендуется использовать контейнер** для сборки (консистентность окружения)
- ⚠️ Локальная сборка возможна только если у вас установлен Node.js 22+ (совпадает с версией в контейнере)
...
### NPM команды (рекомендуется использовать контейнер)

**Режим разработки:**
```bash
# Dev-сервер уже запущен автоматически при старте контейнера
# Проверить статус
...
**Режим продакшена (сборка):**
```bash
# Собрать статические файлы для продакшена
docker-compose exec node npm run build

# Сборка будет в public/build/
```
...
# Запустить watch режим (альтернатива dev-серверу)
docker-compose exec node npm run watch
```

### NPM команды (локально - только если Node.js 22+ установлен)

⚠️ **Не рекомендуется** - используйте контейнер для консистентности!
